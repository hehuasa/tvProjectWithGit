    ...
    >
      ...
      >
        ...
        <Card title="设备选择类型" extra={rangeExtra}>
          {alarmRange === 1 ? (
            <Tabs defaultActiveKey="1" className={styles.infoJudgment}>
...
@connect(({ template }) => ({
  ...
  msgRuleMenus: template.msgRuleMenus,
}))
@Form.create()
export default class TableList extends PureComponent {
  ...
  getFunctionMenus = () => {
    ...
    dispatch({
      type: 'template/getMsgRuleMenus',
      payload: { parentCode: functionInfo.functionCode },
    });
  };
  // 判断是否有该功能权限
  judgeFunction = (functionName) => {
    const { msgRuleMenus } = this.props;
    const arr = msgRuleMenus.filter(item => item.functionName === functionName);
    return arr.length > 0;
  };
  renderSimpleForm() {
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            ...
            <StandardTable
              ...
              rowKey="dealRuleID"
              scroll={{ x: 1500 + win3 * columns.length - 1 }}
