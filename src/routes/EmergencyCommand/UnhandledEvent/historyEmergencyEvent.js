...
@connect(({ panelBoard, emergency, user, homepage, video, global, accessControl, loading }) => ({
  panelBoard,
  historyEventPage: emergency.historyEventPage,
...
export default class Analysis extends PureComponent {
  ...
  componentDidMount() {
    this.historyEventPage(1, 10);
  }
  historyEventPage = (pageNum, pageSize) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'emergency/historyEventPage',
      payload: { pageNum, pageSize, isDrill: 0, isQuery: true, fuzzy: true },
    });
  };
  //  处理事件
   onRowClick = (record) => {
          ...
          <div className={styles.tableList}>
            ...
            <div className={styles.eventTable}>
              <Table
                loading={loading}
                discheckeble
                dataSource={this.props.historyEventPage.data}
                columns={columns}
                rowKey={record => record.eventID}
                pagination={{
                  onChange: this.historyEventPage,
                  ...this.props.historyEventPage.pagination,
                }}
                scroll={{ x: 2720 }}
              />
            </div>
