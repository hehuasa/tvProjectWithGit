...
@connect(({ commonResourceTree, map, sidebar, loading, alarm, panelBoard, homepage, user, constantlyData, global, constructMonitor, paSystem, video, mapRelation }) => {
  return {
    treeData: commonResourceTree.treeData,
    appendData: commonResourceTree.appendData,
    contextMenu: commonResourceTree.contextMenu,
    selectedNodes: commonResourceTree.selectedNodes,
    resourceInfo: commonResourceTree.resourceInfo,
    resInfo: commonResourceTree.resInfo,
    resourceIDs: homepage.resourceIDs,
    videoFooterHeight: homepage.videoFooterHeight,
    refreshTime: commonResourceTree.refreshTime,
    checkedNodesObj: commonResourceTree.checkedNodesObj,
    resourceGroupByArea: commonResourceTree.resourceGroupByArea,
    clusterRes: commonResourceTree.clusterRes,
...
export default class Trees extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'commonResourceTree/queryCheckedNode',
    });
    // // 选中报警预览
    ...
    if (parentNode) {
      if (parentNode.loadResource) {
        this.props.dispatch({
          ...
          payload: {
            param: ['orgID=orgID', 'treeID=parentTreeID'],
            asyncUrl: 'emgc/resource/resourceInfo/getOftenResource',
          },
        });
      } else {
        this.props.dispatch({
  ...
  onRightClick = (e, treeId, treeNode) => {
    ...
    if (resourceID) {
      this.props.dispatch({
        ...
        payload: { position, show: true, resourceID, treeNode },
      });
    }
  };
  ...
  // 获取ajax参数
  getAutoParam = () => {
    return this.props.ajaxParam.param;
  };
  // 获取ajaxUrl
  getAsyncUrl = () => {
    return this.props.ajaxParam.asyncUrl;
  };
  ...
  render() {
    ...
    return (
      !loading.effects['commonResourceTree/fetch'] ? (
